...
// ANswer: the following line of code is setting a variable as an API key to an environment variable that we set in the terminal. This api key is provided by google.
...
app.get('/api/v1/books/find', (req, res) => {
  ...
  // query is set as a empty string for the user to input on the client side. And if the user is searching for the title, author, and isbn it requests it. Its vuilt out to check if any information exists in the fields. The title, author and isbn are used to create the query
  let query = ''
  ...
  // superagent is a middleware that makes the api call. Some other libraries are: Request, Axios and Got
  superagent.get(url)
    .query({'q': query})
    .query({'key': API_KEY})
    .then(response => response.body.items.map((book, idx) => {
      ...
      // destructuring turns properties of an object into a variable.
      let { title, authors, industryIdentifiers, imageLinks, description } = book.volumeInfo;

      // COMMENT: What is the purpose of the following placeholder image? 
      // its a default image incase there are no books to find
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // COMMENT: Explain how ternary operators are being used below. 
      // the ternary operators are used as if/else statements, for example: if a title exists from the API query the title is displayed, if there is no title it will return "No title available". and it does the same thing for all the properties.
...
// The route above was finding books using the title, author and isbn, this route find the books only using the isbn number.
